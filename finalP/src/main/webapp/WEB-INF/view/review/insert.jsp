...
<el-row :gutter="30">
	<el-col :span="10">	
		<el-table :data="attendanceList" class="w-fit" @row-click="rowClick"
			...
			<el-table-column prop="customerAttendanceEnterTime" label="방문일자"></el-table-column> 
		</el-table>
   	<br>
	...
	<el-alert title="작성 시 확인해주세요." type="warning" >
	   	...
	   	<p>출석 정보는 최근 5개의 정보만 조회됩니다.</p><br>
	</el-alert>
	...
	<el-col :span="14">
		<el-form label-position="right" ref="form" label-width="200px" status-icon class="max-w-lg"
			...
			<el-form-item label="방문번호">
				<el-input readonly name="customerAttendanceNo" v-model="customerAttendanceNo"/>
			</el-form-item>			
...
<c:set var="script">
	...
	methods: {
		submit() {
			...
			} else if(this.customerAttendanceNo.length < 1){
				alert('출석정보를 선택해주세요');
			} else {
		...
		rowClick(row, column){	// 상세보기
			...
			if(column.property){
				...
				this.indexNo = document.getElementById('indexNo').value;
			}
     	},	
	},
</c:set>
