...
public class ReservationController extends DefaultController {
    ...
    public String reservationList(
        ...
        Map<String, Object> list = reservationService.selectReservationList(
            paramMap
        );
    ...
    @ResponseBody
    public ResponseEntity<List<ProgramDate>> reservationInfos(
        @PathVariable int program_no
    ) {
    ...
    @ResponseBody
    public ResponseEntity<?> insertReservation2(
        @RequestBody ProgramReservation reservation,
        HttpSession session
    ) {
    ...
    public String deleteReservation(
        ...
        System.out.println(
            targetYear + " " + targetMonth + " " + targetDay + " " + "<--날짜"
        );

        reservationService.deleteReservation(reservation);
        System.out.println(
            reservation.getProgramReservationNo() + "<-- 예약번호"
        );
        return (
            "redirect:/reservationList?targetYear=" +
            targetYear +
            "&targetMonth=" +
            targetMonth +
            "&targetDay=" +
